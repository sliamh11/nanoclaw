...
|---|---|---|---|---|
| `AAG-011` | Cross-platform warden hooks | Warden gate hooks are bash scripts that work on macOS/Linux but not Windows natively. The gate logic should be a single Python module that both Claude Code bash shims and Codex hooks call into, using git for project root detection and no OS-specific path assumptions. | Windows users cannot use warden gates. Codex hook parity requires maintaining parallel implementations instead of sharing logic. | A single `scripts/warden_gate.py` implements all gate/invalidation logic, bash hooks are thin shims calling it, Codex hooks call the same module, and `scripts/codex_warden_hooks.py check` passes on macOS/Linux/Windows. |
