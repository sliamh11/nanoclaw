---
name: x-integration
description: X (Twitter) integration for Deus. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** Deus v1.0.0. Directory structure may change in future versions.
...
1. **Deus is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep deus (macOS) or systemctl --user status deus (Linux)
```
...
| `DEUS_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/deus.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into Deus, make the following modifications:
...
All paths below are relative to project root (`DEUS_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep deus  # macOS — should show PID and exit code 0 or -
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/deus.log | tail -20

# Script errors
grep -i "error\|failed" logs/deus.log | tail -20
```
...
docker run deus-agent ls -la /app/src/skills/
```
