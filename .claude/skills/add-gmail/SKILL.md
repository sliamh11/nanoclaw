...
This skill adds Gmail support to Deus — either as a tool (read, send, search, draft) or as a full channel that polls the inbox.
...
>
> `@Deus check my recent emails` or `@Deus list my Gmail labels`

### Test channel mode (Channel mode only)

Tell the user to send themselves a test email. The agent should pick it up within a minute. Monitor: `tail -f logs/deus.log | grep -iE "(gmail|email)"`.
...
5. Rebuild: `cd container && ./build.sh && cd .. && npm run build && launchctl kickstart -k gui/$(id -u)/com.deus` (macOS) or `systemctl --user restart deus` (Linux)
...
8. Rebuild: `cd container && ./build.sh && cd .. && npm run build && launchctl kickstart -k gui/$(id -u)/com.deus` (macOS) or `systemctl --user restart deus` (Linux)
